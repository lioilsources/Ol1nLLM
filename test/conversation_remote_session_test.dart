...
void main() {
  group('canReuseRemoteSession', () {
    ...
    test('refuses after switching to a sibling branch tip', () {
      // The killer case: 'a2' has no children, so a forksOnNextSend-style
      // check would wrongly report "linear, reuse it" — yet the server's
      // deque still holds the a1 branch.
      final conv = _conv(
        messages: [
          _msg('u1'),
    ...
    test('refuses when the leaf moved back to fork from an older turn', () {
      final conv = _conv(
        ...
        activeLeafId: 'u1', // jumped back; next send branches
        remoteSessionId: 's-1',
        remoteLeafId: 'a1',
      );
      expect(conv.canReuseRemoteSession, isFalse);
  ...
  group('persistence', () {
    ...
    test('clearRemoteSession nulls both halves', () {
      final conv = _conv(
        messages: const [],
        remoteSessionId: 's-1',
        remoteLeafId: 'u1',
      ).copyWith(clearRemoteSession: true);
      expect(conv.remoteSessionId, isNull);
      expect(conv.remoteLeafId, isNull);
    });

    test('omits the fields entirely when unset', () {
      final json = _conv(messages: const []).toJson();
    ...
    test('loads a legacy conversation that predates all new fields', () {
      // What an old Hive blob looks like: no remote session, no sources.
      // Must load without migration.
      final legacy = {
        'id': 'c-old',
        'title': 'staré',
    ...
    test('carries citations on the message they belong to', () {
      ...
      final restored = Conversation.fromJson(
        conv.toJson(),
      ).messages.firstWhere((m) => m.id == 'a1');
      expect(restored.sources, hasLength(1));
      expect(restored.sources.first.label, 'zhuangzi (část 233/488)');
      expect(restored.sources.first.distance, closeTo(0.17, 1e-9));
    });
  });
}
