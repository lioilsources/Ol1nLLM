...
/// ingests do not carry every key (`title` in particular is often absent).
class LibrarySource {
  /// Work the chunk comes from, e.g. `Tao te ťing`.
  final String work;

  /// Human title from ingest metadata, when it differs from [work].
  final String? title;
  ...
  /// Never render this as a percentage without inverting it first.
  ...
  /// Label to show in the sources list: the title when it adds something,
  /// otherwise the work id.
  String get label {
    final t = title?.trim();
