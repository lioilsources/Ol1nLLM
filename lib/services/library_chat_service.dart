...
/// exposed as chat.ol1n.com). Retrieves from a ChromaDB corpus of ~29 works
...
///    the session id back.
///  * **The SSE stream is not OpenAI-shaped**: `data: {"delta": "..."}` per
///    token and a terminal `data: {"done": true, sources, session_id, model}`.
///    There is no `[DONE]` sentinel and no `finish_reason`.
class LibraryChatService extends ChatBackend {
  ...
  /// the response headers and the first token: embedding + Chroma (~0.5 s)
  /// plus prefill of a 9–12k token prompt, which runs into minutes on the
  /// GB10. Once decoding starts, deltas arrive every few hundred ms — so five
  /// minutes of silence genuinely means dead, while vLLM's 120 s would abort
  /// perfectly healthy queries.
  ...
  Map<String, String> get _headers => {
    ...
    // Cloudflare Access is optional here: it is not enabled on
    // chat.ol1n.com yet, and LAN development bypasses Cloudflare entirely.
  ...
  /// server builds its own prompt from `prompts/librarian_cs.md` plus the
  /// work catalog and keeps its own history. See [ChatBackend.chat].
  @override
  Stream<ChatEvent> chat(
    ...
    if (response.statusCode != 200) {
      ...
      // is up while the RAG server itself is down more often than not.
  ...
  /// a failure here only leaves an orphaned deque that ages out on restart.
  @override
